...
class Controller extends BaseController
{
    ...
    public function remove_hub_file($name)
    {
        ...
        if (null != $get_file && ( ($get_file['user_id'] == \Auth::user()->id ) ) ) {
            ...
            if($get_file['type']=="offer"){
                $offer=\App\Offer::where('id',$get_file['type_id'])->firstOrFail();
                if(!$offer->editable())return 0;
            }
            if($get_file['type']=="project"){
                $project=\App\Project::where('id',$get_file['type_id'])->firstOrFail();
                if(!$project->editable())return 0;
            }
            if($get_file['type']=="message" && $get_file['used_at']!=null ){
                return 0;
            }
    ...
    public function notify_me($message="لديك تنبيه جديد",$url=null){
